...
class InputContainer:
    ...
    def numerical_widget(cls, key, custom_label=None, step=0.1, min=None, max=None) -> dict:
        ...
        if min is None and max is None:
            ...
            return dict(value=cls.defaults()[key], label=label, step=step, min=min, max=max)
...
class DataInputs(InputContainer):
    ...
    def widget_representation(cls) -> dict:
        return dict(
            ...
            voxel_size_z=cls.numerical_widget("voxel_size_z", custom_label="Voxel size (z)"),
            voxel_size_y=cls.numerical_widget("voxel_size_y", custom_label="Voxel size (y)"),
            voxel_size_x=cls.numerical_widget("voxel_size_x", custom_label="Voxel size (x)"),
        )
...
class DetectionInputs(InputContainer):
    ...
    def widget_representation(cls) -> dict:
        return dict(
            ...
            ball_xy_size=cls.numerical_widget("ball_xy_size", custom_label="Ball filter (xy)"),
            ball_z_size=cls.numerical_widget("ball_z_size", custom_label="Ball filter (z)"),
            ball_overlap_fraction=cls.numerical_widget("ball_overlap_fraction", custom_label="Ball overlap"),
            log_sigma_size=cls.numerical_widget("log_sigma_size", custom_label="Filter width"),
            n_sds_above_mean_thresh=cls.numerical_widget("n_sds_above_mean_thresh", custom_label="Threshold"),
            soma_spread_factor=cls.numerical_widget("soma_spread_factor", custom_label="Cell spread"),
            max_cluster_size=cls.numerical_widget("max_cluster_size", custom_label="Max cluster", min=0, max=10000000),
        )
...
class MiscInputs(InputContainer):
    ...
    def widget_representation(cls) -> dict:
        return dict(
            ...
            n_free_cpus=cls.numerical_widget("n_free_cpus", custom_label="Number of free CPUs"),
