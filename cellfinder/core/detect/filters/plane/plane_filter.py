...
class TileProcessor:
    ...
    # threshold_value
    n_sds_above_mean_thresh: float
    n_sds_above_mean_local_thresh: float
    ...
    def __init__(
        ...
        n_sds_above_mean_thresh: float,
        n_sds_above_mean_local_thresh: float,
        local_thresh_tile_size: float | None,
        log_sigma_size: float,
        soma_diameter: int,
        torch_device: str,
        dtype: str,
        ...
        self.n_sds_above_mean_local_thresh = n_sds_above_mean_local_thresh
        if local_thresh_tile_size:
            self.local_threshold_tile_size_px = int(
                round(soma_diameter * local_thresh_tile_size)
            )
    ...
    def get_tile_mask(
        ...
        _threshold_planes(
            ...
            self.n_sds_above_mean_local_thresh,
...
def _threshold_planes(
    ...
    n_y_tiles = max(y // stride - 1, 1)
    n_x_tiles = max(x // stride - 1, 1)
    do_tile_y = n_y_tiles >= 2
    do_tile_x = n_x_tiles >= 2
    # num edge pixels dropped b/c moving by a stride would move tile off edge
    y_rem = y % stride
    x_rem = x % stride
    ...
    if local_threshold_tile_size_px >= 2 and (do_tile_y or do_tile_x):
        ...
        threshold = mean + n_sds_above_mean_local_thresh * std
        ...
        # original size. So we add 2 strides and then chop off the excess.
        # We center it because of 50% overlap, the first tile is actually
        # centered in between the first two strides
        if do_tile_y:
            repeats = (
                torch.ones(n_y_tiles, dtype=torch.int, device=torch_device)
                * stride
            )
            repeats[0] = 2 * stride
            repeats[-1] = 2 * stride
            output_size = (n_y_tiles + 2) * stride

            threshold = threshold.repeat_interleave(
                repeats, dim=1, output_size=output_size
            )
            offset = (stride - y_rem) // 2
            threshold = threshold[:, offset : y + offset, :]

        if do_tile_x:
            repeats = (
                torch.ones(n_x_tiles, dtype=torch.int, device=torch_device)
                * stride
            )
            repeats[0] = 2 * stride
            repeats[-1] = 2 * stride
            output_size = (n_x_tiles + 2) * stride

            threshold = threshold.repeat_interleave(
                repeats, dim=2, output_size=output_size
            )
            offset = (stride - x_rem) // 2
            threshold = threshold[:, :, offset : x + offset]
